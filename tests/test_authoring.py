...
class AuthoringTests(unittest.TestCase):
    def test_scaffold_profile_pack_creates_expected_shape(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("type: log", profile_text)
    ...
    def test_check_profile_pack_rejects_actionless_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            pack_dir = Path(temp_dir) / "profiles" / "example" / "daily"
            _write_real_pack(pack_dir)
            ...
            self.assertIn("profile must contain at least one Action", result.errors)
